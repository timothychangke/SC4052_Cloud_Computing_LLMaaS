...
\emph{Display advertising} presupposes
...
\emph{Interruption-based formats} monetise attention by suspending the user's task for a fixed
...
merely an aesthetic complaint; it directly degrades the property that makes conversational interfaces valuable in the first place.
...
while a mandatory disclosure marker= must accompany every commercial mention to satisfy
...
suited to conversational interfaces. Clicks are sparse and a click measures only
...
chat assistant is perceived as a neutral helper, and any
...
\emph{Sensitivity gating.} Certain conversational contexts are
