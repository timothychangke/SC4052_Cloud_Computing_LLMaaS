...
analogous reasoning, place that product naturally within a generated scene---on
a shelf, in a character's hand, within the environment the user described---rather
...
not as an afterthought---and this project therefore treats them as first-class
...
\begin{enumerate}[label=(\roman*), leftmargin=2.5em]
  ...
  \item \textbf{Semantic ad matching with budget-aware eligibility.} The
    ...
    eligibility check---verifying that the parent campaign is active, within
  ...
  \item \textbf{Adaptive per-ad context optimisation.} Each advertisement in
    the inventory carries a dynamic presentation-guidance document---its
    \emph{ad context}---which is injected into the response-synthesis prompt
  ...
  \item \textbf{Advertiser portal with campaign and budget management.} A
    ...
    campaign. Campaigns follow an explicit lifecycle---\texttt{draft},
    \texttt{active}, \texttt{paused}, \texttt{completed}---with
...
for the current scale---tens of thousands of ads, moderate request
...
the platform-wide and advertiser-scoped dashboards. Section~\ref{sec:implementation}
