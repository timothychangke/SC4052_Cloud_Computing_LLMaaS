...
agreement. The pay-off is that the two human workstreams --- a backend
...
tags, a bid in CPM, a remaining budget, and two optional fields ---
\texttt{similarity} and \texttt{score} --- that are populated in
...
\emph{label stability}: the Haiku-tier extractor must return a
...
context extractor --- whether a timeout, a malformed JSON response, or
a schema validation error --- causes the orchestrator to substitute a
